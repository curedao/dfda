  ...
  appMode: {
    ...
    isTesting: function(){
      ...
      if(qm.appMode.isBackEnd()){
        ...
        var testing = env.NODE_ENV === "testing" || env.NODE_ENV === "mocha" || env.NODE_ENV === "test";
        if(testing){qm.appMode.mode = qm.appMode.modes.testing}
        return testing;
      }
      ...
      for(var key in params){
        if(params.hasOwnProperty(key)){
          if(url.indexOf(key + '=') === -1){
            ...
            if(value === null){
              console.error("Not adding null param " + key);  // Don't use qmLog here because it's called by the logger -> infinite loop
              continue;
            }
            if(url.indexOf('?') === -1){
